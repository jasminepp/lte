...
class vv9(nn.Module):
    ...
    def __init__(self, encoder_spec, imnet_spec=None, hidden_dim=256):
        ...
        # self.coef = nn.Conv2d(self.encoder.out_dim, hidden_dim, 1, padding=0)
        self.freq = nn.Conv2d(self.encoder.out_dim, hidden_dim, 3, padding=1)
        # self.hrconv = nn.Conv2d(self.encoder.out_dim, 3, 3, padding=1)
        # self.featureFusionmodule = FeatureFusionModule(hidden_dim, hidden_dim, 1, hidden_dim)

        # self.imnet = models.make(imnet_spec, args={'in_dim': hidden_dim})
        self.out = MLP(( 2 * (6 - 1) +hidden_dim)*4+2, hidden_dim, 256)
    ...
    def query_rgb(self, coord, cell=None):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                rel_coord_n = rel_coord

                preds.append(q_freq)

                for n in range(5):
                    rel_coord_n_clip = torch.clip(rel_coord_n - 1e-6, -1., 1.)
                    rel_coord_n_d = (((rel_coord_n_clip + 1.) * 2 ** n).floor() % 2)
                    rel_coords.append(rel_coord_n_d)
        ...
        grid = torch.cat([*rel_coords, *preds, rel_cell], dim=-1)
        ...
        feat = x
        result = self.fc2(F.gelu(self.fc1(feat)))
...
class WaveletAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        combined_dwt = F.interpolate(combined_dwt.cuda(), scale_factor=2, mode='bilinear', align_corners=False)  # or bicubic bilinear
